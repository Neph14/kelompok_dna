...
onSnapshot(collection(db, "peminjaman"), (snapshot) => {
  ...
  snapshot.forEach((documentDoc) => {
    ...
    let tombolAksiHtml = "";
    if (data.status_peminjaman === "MENUNGGU_KONFIRMASI") {
      tombolAksiHtml = `
  ...
  document.getElementById("txt-total-konfirmasi").innerText = `${totalMenunggu} Request`;
...
function renderCharts(barData, pieData) {
  // 1. Setup Bar Chart (12 Bulan)
  const ctxBar = document.getElementById("chartBatang").getContext("2d");
  if (barChartInstance) barChartInstance.destroy(); // Hapus instance lama agar tidak tumpang tindih
      ...
      datasets: [
        {
          label: "Buku Dipinjam",
          data: barData,
          backgroundColor: "#EA6113", // Warna Litera Oranye
          borderRadius: 5,
        },
      ],
    },
  ...
  // 2. Setup Pie/Donut Chart (Program Studi)
      ...
      datasets: [
        {
          ...
          backgroundColor: ["#EA6113", "#36A2EB", "#FFCE56", "#4BC0C0", "#9966FF"],
        },
      ],
    },
    options: {
...
window.prosesAksi = async (id, statusBaru) => {
  ...
  try {
    if (statusBaru === "DIPINJAM") {
      ...
      alert("Buku Berhasil Diserahkan! Hitung mundur 1 hari aktif di mobile mahasiswa.");
...
// ================= BACA DATA REALTIME KOLEKSI BUKU (DENGAN ADAPTIVE BACKGROUND & CRUD EFFECT) =================
...
onSnapshot(collection(db, "koleksi_buku"), (snapshot) => {
    ...
    snapshot.forEach((documentDoc) => {
        ...
        const coverBuku = data.imageUrl || "https://via.placeholder.com/150x210?text=No+Cover";

        // RENDER CARD DENGAN DYNAMIC BACKDROP BLUR & OVERLAY TOMBOL CRUD DI HOVER EFFECT
        htmlGridBuku += `
    ...
    if (gridBuku) {
        gridBuku.innerHTML = htmlGridBuku || `<div class="col-span-full text-center text-gray-400 italic py-8">Belum ada koleksi buku di database.</div>`;
    }
    if (badgeTotalBuku) {
        badgeTotalBuku.innerText = `${totalBuku} Judul Buku`;
    }
});
...
// 1. Membuka Modal (Mode Tambah Baru)
...
// 3. Mempersiapkan Form untuk Mode Edit
window.prepareEditBuku = (id, judulB64, penulisB64, kategoriB64, url) => {
    document.getElementById("input-buku-id").value = id;
    document.getElementById("input-buku-judul").value = decodeURIComponent(atob(judulB64));
    document.getElementById("input-buku-penulis").value = decodeURIComponent(atob(penulisB64));
    document.getElementById("input-buku-kategori").value = decodeURIComponent(atob(kategoriB64));
...
// 4. Handle Submit Form (CREATE dan UPDATE)
if (formBuku) {
    formBuku.addEventListener("submit", async (e) => {
        ...
        try {
            if (idBuku) {
                // UPDATE
                await setDoc(doc(db, "koleksi_buku", idBuku), dataBuku, { merge: true });
...
window.hapusBuku = async (id, judulB64) => {
    ...
    if (confirm(`Apakah kamu yakin ingin menghapus buku "${judul}" dari perpustakaan?`)) {
        try {
...
// ================= BACA DATA REALTIME KOLEKSI USERS =================
...
onSnapshot(collection(db, "users"), (snapshot) => {
  ...
  snapshot.forEach((documentDoc) => {
    ...
    htmlTabelMhs += `
            <tr class="hover:bg-gray-50 transition text-sm">
                ...
                <td class="px-6 py-4 font-mono text-gray-600">${npmMhs}</td>
  ...
  if (tabelMahasiswa) {
    tabelMahasiswa.innerHTML =
      htmlTabelMhs ||
      `<tr><td colspan="5" class="px-6 py-4 text-center text-gray-400 italic">Belum ada mahasiswa terdaftar</td></tr>`;
  }
